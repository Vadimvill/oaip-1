...
void output_choice()
{
		printf("Print what you want to do (1 - call a student/2 - swap students/3 - view current list/4 - add student)\n");	
}
...
void output_result(const int result_arr[])
{
	printf("The result is:\n");
	for (int i = 0; students_passed[i].last_name != NULL; i++)
	{
		printf("%d. %s  %s - %d\n", i + 1, students_passed[i].last_name, students_passed[i].last_name,result_arr[i]);
	}
}
